...
function parseLaoZhangError(
    ...
    try {
        ...
        if (errorData.error?.message) {
            ...
            if (
                apiMessage.includes("无可用渠道") ||
                apiMessage.includes("无可用")
            ) {
    ...
    return {
        ...
        async generate(params: GenerateParams): Promise<SavedFileInfo[]> {
            ...
            if (isNanoBananaPro) {
                ...
                const payloadFamily =
                    mapping?.payloadFamily || "google_native_image";
                ...
                console.log(
                    "[LaoZhang Image] Отправка Google Native Format запроса:",
                    {
                ...
                const data =
                    (await response.json()) as LaoZhangGoogleNativeResponse;
            ...
            };
            {
                const mapping = getLaoZhangModelMapping(params.model);
                const payloadFamily =
                    mapping?.payloadFamily || "openai_compatible_chat";
                const preflight = validateLaoZhangPayload(
                    params.model,
                    payloadFamily,
                    requestBody,
...
export function createLaoZhangProvider(config: LaoZhangConfig): MediaProvider {
    ...
    return {
        ...
        async checkTaskStatus(
            ...
            return await videoProvider.checkTaskStatus!(taskId, context);
        },
        ...
        async getTaskResult(
            ...
            // Только для видео провайдера
            return await videoProvider.getTaskResult!(taskId, context);
        },
    };
}
