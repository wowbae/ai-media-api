...
export function createKieAiVeo3Provider(config: KieAiConfig): MediaProvider {
  ...
  async function createVeo3Task(
    ...
    if (!modelConfig || modelConfig.provider !== "kieai") {
      throw new Error(`Модель ${params.model} не поддерживается Kie.ai Veo 3.1`);
    }
    ...
    if (params.seed !== undefined) {
      const seedNum =
        typeof params.seed === "number" ? params.seed : parseInt(params.seed, 10);
    ...
    console.log("[Kie.ai Veo 3.1] Задача создана:", responseData.data.taskId);
    return responseData;
  }
  ...
  async function getTaskResultFromAPI(
    ...
    let state: "waiting" | "queuing" | "generating" | "success" | "fail" = "generating";
    if (apiResponse.data.response?.resultUrls && apiResponse.data.response.resultUrls.length > 0) {
      state = "success";
  ...
  return {
    ...
    async checkTaskStatus(taskId: string): Promise<TaskStatusResult> {
      ...
      if (taskData.response?.resultUrls) {
        ...
        try {
          ...
          resultUrls =
            resultData.resultUrls || (resultData.videoUrl ? [resultData.videoUrl] : []);
        } catch (error) {
          console.error("[Kie.ai Veo 3.1] Ошибка парсинга resultJson:", error);
        }
      }
    ...
    async getTaskResult(taskId: string): Promise<SavedFileInfo[]> {
      ...
      console.log(`[Kie.ai Veo 3.1] Файлы сохранены: ${files.length} файлов`);
      return files;
    },
  };
}
