...
class TaskTrackingService {
  ...
  async startTracking(params: {
    ...
    if (this.trackedTasks.has(key)) {
      console.log(`[TaskTracking] Задача уже отслеживается: requestId=${params.requestId}`);
      return;
    }
    ...
    console.log(`[TaskTracking] 🎯 Начато отслеживание: requestId=${params.requestId}, taskId=${params.taskId}`);

    // Запускаем периодическую проверку статуса
    this.startStatusCheckForTask(key, task);
  }
  ...
  stopTracking(requestId: number): void {
    ...
    this.stopStatusCheckForTask(key);
    this.trackedTasks.delete(key);
    console.log(`[TaskTracking] ⏹️ Остановлено отслеживание: requestId=${requestId}`);
  }

  /**
   * Запустить периодическую проверку статуса для задачи
   */
  private startStatusCheckForTask(key: string, task: TrackedTask): void {
    ...
    this.statusCheckIntervals.set(key, intervalId);
  }

  /**
   * Остановить периодическую проверку статуса для задачи
   */
  private stopStatusCheckForTask(key: string): void {
    const intervalId = this.statusCheckIntervals.get(key);
    if (intervalId) {
      clearInterval(intervalId);
      this.statusCheckIntervals.delete(key);
    }
  }
  ...
  private async checkTaskStatus(key: string, task: TrackedTask): Promise<void> {
    ...
    if (elapsed > this.MAX_CHECK_TIME) {
      console.warn(`[TaskTracking] ⏰ Таймаут задачи: requestId=${task.requestId}`);
    ...
    try {
      ...
      if (!provider.checkTaskStatus) {
        console.error(`[TaskTracking] Провайдер не поддерживает checkTaskStatus: ${provider.name}`);
        this.stopTracking(task.requestId);
        return;
      }
      ...
      console.log(`[TaskTracking] 📊 Статус задачи: requestId=${task.requestId}, status=${status.status}, checkCount=${task.checkCount}`);
      ...
      console.error(`[TaskTracking] Ошибка проверки статуса: requestId=${task.requestId}:`, 
  ...
  private async handleTaskTimeout(task: TrackedTask): Promise<void> {
    ...
    console.error(`[TaskTracking] ⏰ Таймаут задачи: requestId=${task.requestId}`);
  }
  ...
  getStats(): {
    ...
    return {
      ...
      activeChecks: this.statusCheckIntervals.size,
    };
  }
}
